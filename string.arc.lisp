  ...
  (let test (testify sep)
    (let rec (afn (cs toks tok)
               (if (no cs)         (consif tok toks)
                   (funcall test (car cs)) (self (cdr cs) (consif tok toks) nil)
                                   (self (cdr cs) toks (cons (car cs) tok))))
      (rev (map [coerce _ 'string]
                (map #'rev (funcall rec (coerce s 'cons) nil nil)))))))
  ...
  (let test (testify sep)
    (let rec (afn (cs tok)
               (if (no cs)         (list (rev tok))
                   (funcall test (car cs)) (list cs (rev tok))
                                   (self (cdr cs) (cons (car cs) tok))))
      (rev (map [coerce _ 'string]
                (funcall rec (coerce s 'cons) nil))))))
  ...
  (catch
    (if (isa pat 'fn)
        (for i start (- (len seq) 1)
          (when (funcall pat (ref seq i)) (throw i)))
...
(def blank (s) 
  (with (find #'find whitec #'whitec)
   (funcall ~find ~whitec s)))
...
(def num (n (o digits 2) (o trail-zeros nil) (o init-zero nil))
  (withs (comma
          ...
          (let a (abs n)
            (if (< digits 1)
                 (funcall comma (roundup a))
                (exact a)
                 (string (funcall comma a)
